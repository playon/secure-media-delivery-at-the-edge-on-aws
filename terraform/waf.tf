...
resource "aws_wafv2_web_acl" "this" {
  name        = "${local.name_prefix}-${local.env_slug}-token-rate-limit"
  ...
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "${local.name_prefix}-${local.env_slug}-token-rate-limit"
  ...
  visibility_config {
    cloudwatch_metrics_enabled = true
    metric_name                = "${local.name_prefix}-${local.env_slug}-web-acl"
    sampled_requests_enabled   = true
  }
}
