...
resource "aws_cloudfront_function" "api_path_rewriter" {
  name    = "${local.name_prefix}-${local.env_slug}-api-path-rewriter"
...
resource "aws_kinesis_stream" "realtime_logs" {
  name             = "${local.name_prefix}-${local.env_slug}-realtime-logs"
  retention_period = 24
...
resource "aws_iam_role" "cf_kinesis" {
  name               = "${local.name_prefix}-${local.env_slug}-cf-kinesis"
...
resource "aws_cloudfront_cache_policy" "with_country_header" {
  name        = "${local.name_prefix}-${local.env_slug}-cache-policy"
  min_ttl     = 0
  default_ttl = 86400
  max_ttl     = 31536000
