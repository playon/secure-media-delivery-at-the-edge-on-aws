...
class Secret:
    ...
    def __init__(self):
        ...
        try:
            primarySecret = Secret.Secret + "_PrimarySecret"
            secondarySecret = Secret.Secret + "_SecondarySecret"
...
def createtoken(attributes,secret_alias,playback_url,**kwargs):
	...
	if "secret_alias" not in kwargs:
	  secret_alias = "primary"
	else:
	  secret_alias = kwargs['secret_alias']
	...
	### Encode token
	encoded_jwt = jwt.encode(jwt_json, key, algorithm="HS256",headers={"kid": "230498151c214b788dd97f22b85410a5"},)
