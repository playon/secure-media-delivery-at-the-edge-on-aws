...
resource "aws_iam_role" "generator" {
  name               = "${local.name_prefix}-${local.env_slug}-generator"
...
resource "aws_lambda_function" "generator" {
  function_name    = "${local.name_prefix}-${local.env_slug}-generator"
...
resource "aws_iam_role" "revoker" {
  name               = "${local.name_prefix}-${local.env_slug}-revoker"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume.json
}
...
resource "aws_lambda_function" "revoker" {
  function_name    = "${local.name_prefix}-${local.env_slug}-revoker"
...
resource "aws_iam_role" "list_revoked" {
  name               = "${local.name_prefix}-${local.env_slug}-list-revoked"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume.json
}
...
resource "aws_lambda_function" "list_revoked" {
  function_name    = "${local.name_prefix}-${local.env_slug}-list-revoked"
...
resource "aws_iam_role" "kvs_cleanup" {
  name               = "${local.name_prefix}-${local.env_slug}-kvs-cleanup"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume.json
}
...
resource "aws_lambda_function" "kvs_cleanup" {
  function_name    = "${local.name_prefix}-${local.env_slug}-kvs-cleanup"
