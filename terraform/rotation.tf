...
resource "aws_iam_role" "sync_keys" {
  name               = "${local.name_prefix}-${local.env_slug}-sync-keys"
...
resource "aws_lambda_function" "sync_keys" {
  function_name    = "${local.name_prefix}-${local.env_slug}-sync-keys"
...
resource "aws_iam_role" "rotation_sfn" {
  name               = "${local.name_prefix}-${local.env_slug}-rotation-sfn"
...
resource "aws_sfn_state_machine" "rotation" {
  name     = "${local.name_prefix}-${local.env_slug}-RotateKeys"
...
resource "aws_iam_role" "rotation_events" {
  name               = "${local.name_prefix}-${local.env_slug}-rotation-events"
...
resource "aws_cloudwatch_event_rule" "rotation_schedule" {
  name                = "${local.name_prefix}-${local.env_slug}-key-rotation"
