...
resource "aws_api_gateway_rest_api" "this" {
  ...
  # VID-3484: policy moved to aws_api_gateway_rest_api_policy.this
  # so its Resource strings can reference this API's .id — TF disallows
  # self-references inside a resource's own attributes, but not from
  # a separate resource that depends on this one. See below.
}
